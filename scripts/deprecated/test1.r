...
latest_game_id <- game_info_df$game_id[nrow(game_info_df)] + 1
yesterday_game_id <- yesterday$game_id[nrow(yesterday)]


remaining_games <- schedule %>% 
  filter(game_id >= latest_game_id & game_status != "Postponed")
...
game_ids <- as.character(seq(latest_game_id, yesterday_game_id, by = 1))
...
scrape.1 <- as.character(seq(2019020751, 2019021073, by = 1))
scrape.1 <- scrape.1[!scrape.1%in%dead_games]

pbp_scrape <- sc.scrape_pbp(games = scrape.1)
