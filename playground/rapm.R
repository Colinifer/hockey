# https://github.com/TopDownHockey/RAPM
...
pbp.corsi_events <- c('Shot', 'Goal', 'Missed Shot', 'Blocked Shot')
pbp.even_strength <- c("3v3", "4v4", "5v5")
pbp.power_play <- c("5v4", "4v5", "5v3", "3v5", "4v3", "3v4")
pbp.home_power_play <- c("5v4", "5v3", "4v3")
pbp.away_power_play <- c("4v5", "3v5", "3v4")
pbp.empty_net <- c("5vE", "Ev5", "4vE", "Ev4", "3vE", "Ev3")
pbp.meaningful_events <- c("Faceoff", "Goal", "Blocked Shot", "Shot", "Miss", "Hit", "Takeaway", "Giveaway", "Change")
stoppages <- c("PGSTR", "PGEND", "ANTHEM", "PSTR", "FAC", "GOAL", "STOP", "PENL", "PEND", "CHL", "GEND", "GOFF", "EISTR", "EIEND", "EGT", "EGPID")
pbp.stoppages <- c('Pre-game Start', 'Pre-game End', 'Anthem', 'Period Start', 'Faceoff', 'Goal', 'Stoppage', 'Penalty', 'Period End', 'Challenge', 'Game End', 'Goalie Off', 'Early Intermission Start', 'Early Intermission End', 'Emergency Goaltender')
...
teams <- all1819 %>%
  group_by(game_date) %>%
  summarize(yesterday_home_team = unique(home_abbreviation), yesterday_away_team = unique(away_abbreviation))
...
all1819$home_home_yesterday <- mapply(grepl, pattern=all1819$home_abbreviation, x=all1819$yesterday_home_team)
all1819$home_away_yesterday <- mapply(grepl, pattern=all1819$home_abbreviation, x=all1819$yesterday_away_team)
all1819$away_home_yesterday <- mapply(grepl, pattern=all1819$away_abbreviation, x=all1819$yesterday_home_team)
all1819$away_away_yesterday <- mapply(grepl, pattern=all1819$away_abbreviation, x=all1819$yesterday_away_team)
...
all1819$shift_change <-
  ifelse(
    lag(all1819$home_on_1) == all1819$home_on_1 &
      lag(all1819$home_on_2) == all1819$home_on_2 &
      lag(all1819$home_on_3) == all1819$home_on_3 &
      lag(all1819$home_on_4) == all1819$home_on_4 &
      lag(all1819$home_on_5) == all1819$home_on_5 &
      lag(all1819$home_on_6) == all1819$home_on_6 &
      lag(all1819$away_on_1) == all1819$away_on_1 &
      lag(all1819$away_on_2) == all1819$away_on_2 &
      lag(all1819$away_on_3) == all1819$away_on_3 &
      lag(all1819$away_on_4) == all1819$away_on_4 &
      lag(all1819$away_on_5) == all1819$away_on_5 &
      lag(all1819$away_on_6) == all1819$away_on_6 &
      lag(all1819$game_score_state) == all1819$game_score_state &
      lag(all1819$period) == all1819$period &
      lag(all1819$home_goalie) == all1819$home_goalie &
      lag(all1819$away_goalie) == all1819$away_goalie &
      lag(all1819$game_id) == all1819$game_id,
    0,
    1
  )
...
all1819$home_pp_expiry <-
  ifelse(
    all1819$strength_state %in% pbp.even_strength &
      lag(all1819$strength_state) %in% pbp.home_power_play &
...
all1819$home_pp_expiry <-
  ifelse((
    lag(all1819$event_type) %in% pbp.stoppages &
      all1819$game_seconds == lag(all1819$game_seconds)
  ) |
    (lag(lag(all1819$event_type)) %in% pbp.stoppages &
       lag(lag(
         all1819$game_seconds
       )) == all1819$game_seconds) |
    (lag(lag(
      lag(all1819$event_type)
    )) %in% pbp.stoppages &
      lag(lag(
        lag(all1819$game_seconds)
      )) == all1819$game_seconds),
  0,
  all1819$home_pp_expiry
  )
...
all1819$away_pp_expiry <-
  ifelse(
    all1819$strength_state %in% pbp.even_strength &
      lag(all1819$strength_state) %in% pbp.away_power_play &
      all1819$shift_change == 1,
    1,
    0
  )
...
home_offense_all <- shifts_grouped %>%
  rename(
    ...
    defense_5 = away_on_5,
    defense_6 = away_on_6,
    Off_Zonestart = Home_OZS,
    Def_Zonestart = Away_OZS,
    Neu_Zonestart = NZS,
    Up_1 = Home_Up_1,
    Up_2 = Home_Up_2,
    Up_3 = Home_Up_3,
  ...
  select(
    ...
    Def_Zonestart,
    Neu_Zonestart,
    offense_goalie = home_goalie,
    defense_goalie = away_goalie,
    Period_1,
    Period_2,
    Period_3,
    BTB,
...
away_offense_all <- shifts_grouped %>%
  rename(
    ...
    defense_5 = home_on_5,
    defense_6 = home_on_6,
    Off_Zonestart = Away_OZS,
    Def_Zonestart = Home_OZS,
    Neu_Zonestart = NZS,
    Up_1 = Away_Up_1,
    Up_2 = Away_Up_2,
    Up_3 = Away_Up_3,
  ...
  select(
    ...
    Def_Zonestart,
    Neu_Zonestart,
    offense_goalie = away_goalie,
    defense_goalie = home_goalie,
    Period_1,
    Period_2,
    Period_3,
    BTB,
